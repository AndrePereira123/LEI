...
class Algoritmo_p:
    def algoritmo_personalizado(grafo:Graph,nodo_inicial:Node,veiculo:Veiculo,suplementos):
        ...
        total_nodes = len(grafo.m_nodes)
        can_stop = False

        while len(open_list) > 0:
            tam_openlist = len(open_list)
            vizinhos_locais_necessidades = False  ## Regista se ha vizinhos com necessidades >0 ou nao 
            vizinhos_possiveis = []                 ## Guarda os caminhos possiveis no caso de nenhum visinho ter necessidades >0
            n = None
            for v in open_list:
                heuristica_vizinho = grafo.heuristica(n_anterior,v,veiculo,False)
                if (heuristica_vizinho == -2): 
                    vizinhos_possiveis.append(v)

                if(heuristica_vizinho > 0):
                    vizinhos_locais_necessidades = True
                    if n == None or g[v] + heuristica_vizinho < g[n] + grafo.heuristica(n_anterior,n,veiculo,False):  
                        heuristica_n = heuristica_vizinho
                        n = v
            
            if (not vizinhos_locais_necessidades):  ##Nenhum visinho tem necessidades >0 logo temos de selecionar um valido (random)
                lista_de_opcoes = []
                for v in vizinhos_possiveis:
                    heuristica_vizinho = grafo.heuristica(n_anterior,v,veiculo,True)
                    if(heuristica_vizinho > 0):                               ##Garantir q podemos acessar o nodo (Mar,ar,combustivel suficiente...)
                ...
                if(lista_de_opcoes != []):
                    ...
                    if(n.reabastecimento == True): 
                        veiculo.combustivel_disponivel = veiculo.autonomia ##sabemos q temos combustivel para ir ate la e q vamos reabastecer logo podemos por logo tanque cheio
                    else:
                        combustivel_necessario = 2 * (veiculo.consumo * distancia)  ##combustivel de ida e volta 
                        veiculo.combustivel_disponivel -= combustivel_necessario

                    var = veiculo.adaptar_tempo_a_condicao_meteriologica(n.condicao_meteriologica)
                    tempo_gasto = (distancia/veiculo.tempo_de_viagem) * var
                    veiculo.tempo_desde_inicio += tempo_gasto

                    resultado = Algoritmo_p.algoritmo_personalizado(grafo,n,veiculo,suplementos_inicial)
    ...
    def algoritmo_total(grafo : Graph,nodo_inicial,veiculos_input  : List[Veiculo],suplementos_requisitados):
            ...
            if (suplementos_requisitados > 0):
                ...
                for v in veiculos[:]: 
                    if (v in veiculos):
                        ...
                        resultado = Algoritmo_p.algoritmo_personalizado(grafo,nodo_inicial,v,v.cap_max)  ##calcula melhor caminho disponivel para 1 veiculo
                        if (resultado != None):
                            (nova_h,custo_caminho,path,suplementos_distribuidos) = resultado
                            if prim_itera == True or nova_h < melhor_heuristica:         ##calculamos a heuristica de todos os veiculos e guardamos a melhor(menor)
                                prim_itera = False
                            ...
                            veiculos_nao_usados.append(v.get_name()) ##removemos veiculos q nao tem caminhos validos disponiveis porque nunca o vão ter
                            veiculos.remove(v)
