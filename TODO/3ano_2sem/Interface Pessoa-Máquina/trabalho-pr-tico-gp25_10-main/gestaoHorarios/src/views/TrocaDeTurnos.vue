...
const selectedAluno = ref(alunoTrocaStore.Aluno_Para_Troca|| ''); 
const idSelectedAluno = ref(alunoTrocaStore.Id_Aluno_Para_Troca || '');
...
// Novo Turno
const selectedNovoTurno = ref('');
const novosTurnos_pre_compatibilidade = ref([]);  // o valor verificado é o "compatibleShifts"

const horarioAluno = ref({});
const error = ref(null);
const TurnoCheio = ref(false); // para verificar se o turno está cheio
const turnoConflitante = ref(false); // para verificar se o turno novo é compatível com o horario (se causa conflitos)
const studentCurrentShifts = ref([]); // turnos atuais do aluno

const compatibleShifts = computed(() => {   // Filtra os turnos compatíveis com o turno selecionado (T ou PL)
  if (!selectedTurno.value) return [];
  
  return novosTurnos_pre_compatibilidade.value.filter(shift => 
    shift.type === selectedTurno.value.type && 
    shift.id !== selectedTurno.value.id
  );
});
...
onMounted(async () => {
  ...
  const allowedPrefix = '/gestao-alunos/troca-turnos';
  const allowedPrefix2 = '/gestao-alunos/listar-alunos-colisoes'

  const hasNoQueryParams = Object.keys(router.currentRoute.value.query).length === 0;

  
  if (hasNoQueryParams && !previousRoute.includes(allowedPrefix) && !previousRoute.includes(allowedPrefix2)) {
    selectedAluno.value = '';
...
watch(selectedNovoTurno, (newShift) => { // Quando o turno é selecionado, verifica-se se está cheio
  if (newShift) {
...
const updateAllocation = async () => {
  try { 
    ...
    const updatedAllocation = {
      studentId: idSelectedAluno.value.toString(),
      shiftId: Number(selectedNovoTurno.value.id), 
      shiftType: selectedNovoTurno.value.type
    };
    ...
    const old_shiftId = selectedTurno.value.id;
    const new_shiftId = selectedNovoTurno.value.id;

    
    const old_response = await fetch(`http://localhost:3000/shifts/${old_shiftId}`);
    if (!old_response.ok) {
      throw new Error('Failed to fetch old shift');
    }
    const old_shift = await old_response.json();
    old_shift.totalStudentsRegistered -= 1;
    
    const old_update = await fetch(`http://localhost:3000/shifts/${old_shiftId}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify(old_shift),
    });

    if (!old_update.ok) {
      throw new Error('Failed to update old shift');
    }

    
    const new_response = await fetch(`http://localhost:3000/shifts/${new_shiftId}`);
    if (!new_response.ok) {
      throw new Error('Failed to fetch new shift');
    }
    const new_shift = await new_response.json();
    new_shift.totalStudentsRegistered += 1;
    
    const new_update = await fetch(`http://localhost:3000/shifts/${new_shiftId}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify(new_shift),
    });
    ...
    window.location.href = `/gestao-alunos/troca-turnos?reload=true&studentId=${idSelectedAluno.value}}`;
  }
  catch (err) { 
      ...
      <div class="form-section">
        ...
        <div class="form-group">
          ...
          <div class="input-group">
            ...
            <button
              class="btn blue"
              @click="router.push('/gestao-alunos/troca-turnos/lista-alunos');"
            >
              Selecionar Aluno
            </button>
          </div>
          ...
          <div style="height: 5vh;"></div>
          <label for="turno" style="color:black;">Turno atual:</label>
          <div class="input-group">
            <select 
              v-model="selectedTurno" 
              class="shift-dropdown"
              :disabled="shifts.length === 0"
              @change="selectedNovoTurno.value = '';"  
            >
              <option value="" disabled>{{!selectedUC ? 'UC por selecionar' : shifts.length == 0 ? 'Aluno não tem turnos desta disciplina' : 'Selecione um turno' }}</option>
              <option 
          ...
          <label for="novoTurno" style="color:black;">Novo Turno:</label>
          <div class="input-group">
            <select 
              v-model="selectedNovoTurno" 
              class="shift-dropdown"
              :disabled="!selectedTurno || compatibleShifts.length === 0"
              @change="handleNovoTurnoSelect"
            >
              <option value="" disabled>{{ !selectedTurno ? 'Turno atual por selecionar' : compatibleShifts.length === 0 ? 'Não há turnos compatíveis' : 'Selecione um novo turno' }}</option>
              <option 
                v-for="shift in compatibleShifts" 
                :key="shift.id" 
                :value="shift"
              >
                {{ shift.name }} | {{ shift.day }} das {{ shift.from }}h às {{ shift.to }}h | Sala: {{ shift.classroom.building.abbreviation }} {{ shift.classroom.name }} | Capacidade: {{ shift.totalStudentsRegistered }}/{{ shift.classroom.capacity }}
          ...
          <div class="action-container">
            ...
            <button class="btn alocar" :disabled="!selectedAluno || !selectedTurno || !selectedNovoTurno" @click="updateAllocation">
              Alocar Aluno
            </button>
            <p v-if="!selectedAluno || !selectedTurno || !selectedNovoTurno" class="warning">
