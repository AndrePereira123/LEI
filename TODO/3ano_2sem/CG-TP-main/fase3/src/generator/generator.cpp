...
int main(int argc, char** argv) {
    if (argc < 2) {
        cerr << "Invalid input.\nUsage: ./generator <plane|box|sphere|cone|ring> <arg_0> ... <arg_n>" << endl;
        return 1;
    }
    ...
    try {
        if (primitive_type == "plane") {
        ...
        } else if (primitive_type == "ring") {
            if (argc != 6) {
                cerr << "Invalid input.\nUsage: ./generator ring <slices> <inner_radius> <outter_radius> <output.3d>" << endl;
                return 1;
            }
            int slices = stoi(argv[2]);
            float inner_radius = stof(argv[3]);
            float outter_radius = stof(argv[4]);
            if (slices <= 0 || inner_radius <= 0 || outter_radius <= 0) {
                cerr << "The values \"slices\", \"inner_radius\" and \"outter_radius\" must be positive." << endl;
                return 1;
            }
            string name_file = argv[5];
            if (!regex_match(name_file, regex("^.*\\.3d$"))) {
            ...
            Ring ring = Ring(slices, inner_radius, outter_radius);
            ring.generate();
            ring.writeToFile(name_file.c_str());
        } else if (primitive_type == "patch") {
            if (argc != 5) {
                cerr << "Invalid input.\nUsage: ./generator patch <patchfile.patch> <tessellation> <output.3d>" << endl;
                return 1;
            }
            ...
            if (tessellation <= 0) {
                cerr << "The value \"tessellation\" must be positive." << endl;
                return 1;
            }
            ...
            if (!regex_match(argv[2], regex("^.*\\.patch$"))) {
                cerr << "The patch file must end with .patch" << endl;
                return 1;
            }

            string patch_file = "../patches/" + string(argv[2]);

            FILE* file = fopen(patch_file.c_str(), "r");
            if (!file) {
                cerr << "The file " << patch_file << " does not exist or could not be open." << endl;
                return 1;
            }
            fclose(file);

            Patch patch = Patch(string(argv[2]), tessellation);
            patch.generate();
            patch.writeToFile(name_file.c_str());            
