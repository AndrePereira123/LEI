...
onMounted(async () => {
  try {
    ...
    const allocationsResponse = await fetch(`http://localhost:3000/allocations?shiftId=${shiftId}`);
    if (!allocationsResponse.ok) {
      throw new Error('Failed to fetch allocations');
    }
    const allocations = await allocationsResponse.json();

    const studentsIds = allocations.map(allocation => String(allocation.studentId))
...
<template>
  <main class="students-list">
    ...
    <div v-else class="Caixa_branca">
      <h1>Alunos de {{ course?.abbreviation }} do turno {{ turno?.name }}</h1>
      ...
      <div class="caixa_2">
        ...
        <div class="pagination">
          ...
          <button
            class="next-page"
            v-if="currentPage < totalPages.value"
            @click="nextPage"
          >
            ▶
          </button>
