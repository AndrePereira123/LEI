...
const totalPages = computed(() => {
  return Math.ceil(allRequests.value.length / itemsPerPage.value);
});
...
const navigateToEnviadas = () => {
  router.push('/atendimento-pedidos/enviadas');
};
  ...
  <main class="shift-requests">
    <div class="Caixa_branca">
      <div style="display: flex; flex-direction: row;width: 100%;align-items: center;justify-content: space-evenly;">
            <h1 style="color: #8B0000;">Pedidos Pendentes</h1>
            <button class="ver-enviadas" @click="navigateToEnviadas">Ver concluídos</button>
        </div>
      ...
      <div v-else-if="allRequests.length > 0" class="requests-list">
        ...
        >
          <p>
            <strong v-if="request.type === 'shift'">Pedido Troca de Turno</strong>
            <strong v-else-if="request.type === 'classroom'">Pedido Troca de Sala</strong>
            - {{ request.date }} |
